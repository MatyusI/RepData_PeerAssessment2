...
```{r}
variablesN <- c("EVTYPE", "FATALITIES", "INJURIES", "PROPDMG", "PROPDMGEXP", "CROPDMG", "CROPDMGEXP")
storm <- storm[variablesN]
dim(storm)
names(storm)
str(storm)
```
...
Which Types of Events are most Harmful for population HEALTH? The variables involved are FATALITIES and INJURIES.
...
The National Weather Service Storm Data documentation describes about Damage. Two variables, PROPDMG (for Property Damage) and CROPDMG (for Crop Damage) are used to represent these losts.
